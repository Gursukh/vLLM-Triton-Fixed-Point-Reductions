...
__global__ void fixed_to_float_kernel(
  ...
  if (base + kVec <= n) {
    #pragma unroll
    for (int i = 0; i < kVec; ++i) {
      const float f = fixed_to_float<FxpInt>(x[base + i]);
      y[base + i] = static_cast<FloatT>(f);
    }
  } else {
    for (int64_t idx = base; idx < n; ++idx) {
      const float f = fixed_to_float<FxpInt>(x[idx]);
...
at::Tensor float_to_fixed_run(
  ...
  switch (x_c.scalar_type()) {
    case at::kFloat:
      switch (int_bits) {
        case 16: launch_f2x<float, int16_t>(x_c, y); break;
        case 32: launch_f2x<float, int32_t>(x_c, y); break;
        case 64: launch_f2x<float, int64_t>(x_c, y); break;
      }
      break;
    case at::kHalf:
      switch (int_bits) {
        case 16: launch_f2x<at::Half, int16_t>(x_c, y); break;
        case 32: launch_f2x<at::Half, int32_t>(x_c, y); break;
        case 64: launch_f2x<at::Half, int64_t>(x_c, y); break;
      }
      break;
    case at::kDouble:
      switch (int_bits) {
        case 16: launch_f2x<double, int16_t>(x_c, y); break;
        case 32: launch_f2x<double, int32_t>(x_c, y); break;
        case 64: launch_f2x<double, int64_t>(x_c, y); break;
      }
      break;
    default:
...
at::Tensor fixed_to_float_run(
  ...
  switch (x_c.scalar_type()) {
    case at::kShort:
      switch (float_bits) {
        case 16: launch_x2f<int16_t, at::Half>(x_c, y); break;
        case 32: launch_x2f<int16_t, float>(x_c, y); break;
        case 64: launch_x2f<int16_t, double>(x_c, y); break;
      }
      break;
    case at::kInt:
      switch (float_bits) {
        case 16: launch_x2f<int32_t, at::Half>(x_c, y); break;
        case 32: launch_x2f<int32_t, float>(x_c, y); break;
        case 64: launch_x2f<int32_t, double>(x_c, y); break;
      }
      break;
    case at::kLong:
      switch (float_bits) {
        case 16: launch_x2f<int64_t, at::Half>(x_c, y); break;
        case 32: launch_x2f<int64_t, float>(x_c, y); break;
        case 64: launch_x2f<int64_t, double>(x_c, y); break;
      }
      break;
    default:
