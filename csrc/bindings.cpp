...
TORCH_LIBRARY(fxpr, m) {
  m.def("float_to_fixed(Tensor x, int int_bits) -> Tensor");
  m.def("fixed_to_float(Tensor x, int float_bits) -> Tensor");
  m.def(
  ...
  m.def(
      ...
      "float eps, int fxp_int_bits) -> Tensor");
  m.def("log_softmax_fxp(Tensor x, int fxp_int_bits) -> Tensor");
  m.def("gemm_fxp(Tensor a, Tensor b, Tensor? bias, int fxp_int_bits) -> Tensor");
  m.def(
      ...
      "float? softmax_scale, int fxp_int_bits, "
